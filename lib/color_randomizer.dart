...
abstract class RandomRGBMethods {
  ...
  Color nextColor();
  Color fromRange(HexRange alphaAndColor);
  Color fromAlphaAndColor(HexRange alpha, HexRange colors);
  Color fromARGBRanges(
      HexRange alpha, HexRange red, HexRange green, HexRange blue);

  /// Gives a random number, depending on the HexRange constraints.
  int randomHexValue(HexRange range) {
    // + 1 due to HexRange being capped to 255,
    // nextInt is exclusive in max value.
    return _r.nextInt(range.max - range.min + 1) + range.min;
  }
}
...
class RandomARGB extends RandomRGBMethods {
  ...
  int randomHexValueFor(ARGBChannel argbChannel) {
    switch (argbChannel) {
      case ARGBChannel.alpha:
        return randomHexValue(alphaConfig);
      case ARGBChannel.red:
        return randomHexValue(redConfig);
      case ARGBChannel.green:
        return randomHexValue(greenConfig);
      case ARGBChannel.blue:
        return randomHexValue(blueConfig);
    }
  }
  ...
  Color fromARGBRanges(HexRange alphaRange, HexRange redRange,
    ...
    return Color.fromARGB(randomHexValue(alphaRange), randomHexValue(redRange),
        randomHexValue(greenRange), randomHexValue(blueRange));
  }

  @override
  Color fromAlphaAndColor(HexRange alphaRange, HexRange colorRange) {
    return Color.fromARGB(
        randomHexValue(alphaRange),
        randomHexValue(colorRange),
        randomHexValue(colorRange),
        randomHexValue(colorRange));
  }

  @override
  Color fromRange(HexRange alphaAndColorRange) {
    return Color.fromARGB(
        randomHexValue(alphaAndColorRange),
        randomHexValue(alphaAndColorRange),
        randomHexValue(alphaAndColorRange),
        randomHexValue(alphaAndColorRange));
  }
